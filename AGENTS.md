...
- **`routes.json` holds SEVERAL variants per line name, not one** (264 features, 48 lines). A line has branches and short turns, and the two ring directions (S41/S42) run different track, so one shape per line name left 6.3% of live vehicles (23 of 367) projecting 300 m-6.5 km off. `pickShape` chooses per vehicle by forecast fit — the forecast is the only evidence of which branch a vehicle is on, because HAFAS names the next stop but not the route to it, and a headsign is not a route (live U7: six destinations, two termini; M5 reaches one depot by two streets). Measured: `badFit` 7.9% -> ~0.3% of segment rebuilds, residual p50 10 -> 4 m, p90 86 -> 19 m. Cost is ~15 ms per poll for 340 vehicles.
- **Build-time shapes are simplified with Douglas-Peucker at 10 m** (`scripts/simplify.mjs`), never decimated to a point count. Nth-point decimation kept every Nth point regardless of geometry — wasting points on straight runs and cutting corners on curves — which is what put residuals in the ~100 m band `SHAPE_FIT_LIMIT_M` was sized around. DP bounds the error by construction: GTFS's 708k points become 117k, and shipping 5x more shapes made the payload *smaller* (76 -> 37 KB gzipped).
- **Do not ship a route variant contained in a longer one** (`isContainedIn`, 50 m tolerance, `scripts/variants.mjs`). 365 of 533 variants once ran entirely along a longer one and told a vehicle nothing the longer one could — a short turn is a *subset* of the full route and projects onto it correctly. Worse, each was simplified independently, so their vertices differ by sub-metre amounts, residuals tied to float noise and `pickShape`'s choice flipped between polls (42% of vehicles changed variant within 60 s, 21% while their declared stop never changed). Pruning: 1,599 corridors -> 264 variants, and ambiguity (choices tied within 0.5 m) 83% -> 53%.
- **Corridor fingerprints sample by DISTANCE, not index** (`corridorKey`). After Douglas-Peucker, vertex density tracks curviness, so index-fraction sampling is not position sampling: the sampled point sat a median 574 m (p90 2.7 km) from the distance-fraction point, and 515 of 530 geometrically identical corridor pairs failed to merge. That made the dedupe near-inert.
- **Rank variants by metres, never by point count.** After DP the two disagree on 14% of pairs; S7's 48 km full-line variant has only 117 points and once survived at rank 11 of 12 — one slot from being dropped and stranding its own subsets. `MAX_VARIANTS_PER_LINE` (24) is a backstop, not a truncation: after pruning no line reaches it, where 41 of 48 hit a cap of 12 before.
