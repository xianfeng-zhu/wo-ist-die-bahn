...
- **`routes.json` holds SEVERAL variants per line name, not one** (533 features, 48 lines). A line has branches and short turns, and the two ring directions (S41/S42) run different track, so one shape per line name left 6.3% of live vehicles (23 of 367) projecting 300 m-6.5 km off. `pickShape` chooses per vehicle by forecast fit — the forecast is the only evidence of which branch a vehicle is on, because HAFAS names the next stop but not the route to it, and a headsign is not a route (live U7: six destinations, two termini; M5 reaches one depot by two streets). Measured effect: `badFit` 7.9% -> 0.25% of segment rebuilds, residual p50 10 -> 4 m, p90 86 -> 19 m, drift median 7 -> 5 m. Cost is ~35 ms per poll for 340 vehicles, so no per-vehicle caching is needed.
...
- **`prepare-data.mjs` caps variants per line** (`MAX_VARIANTS_PER_LINE`, 12) after collapsing by corridor fingerprint (2,179 raw `shape_id`s -> 1,801 corridors; S1 alone has 155 raw shapes). The 12 longest cover the real branches because a short turn is a *subset* of a longer variant and projects onto it correctly. The script prints cost at several caps, so the cap can be retuned without another 600 MB GTFS download; shipping all 1,801 would cost 98 KB gzipped.
- **`pickShape`'s choice is not stable, and length is NOT the fix** (known limitation, documented in `track.ts`). Variants overlap and a 30 s forecast is four points, so the median residual gap to the runner-up is 0.0 m and 42% of vehicles change variant within 60 s. Breaking ties by track length measured *worse* on everything else (drift p90 61 -> 106 m, dwell p90 12 -> 43 s, overspeed 0 -> 8) because it prefers full-line and full-ring variants, and projecting onto a long or closed shape is itself ambiguous. A real fix needs hysteresis on `AnimState` or a tie-break on which variant reaches the declared stop.
