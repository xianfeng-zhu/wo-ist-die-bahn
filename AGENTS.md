...
- **One GTFS shape per line is not enough**: `prepare-data.mjs` keeps the longest shape per line name, so branch variants (S1, M5, tram 12, M2) do not match. `updateSegment` checks the forecast against the track (`SHAPE_FIT_LIMIT`) and follows the forecast points directly when it does not fit — without that guard, 22 of 296 vehicles projected 2-7 km off.
...
- **Coverage**: 61 tests (vehicle 29, motion 19, polyline 4, track 4, hci 5). Pure modules (`motion`, `track`, `vehicle`, `hci` parsing) are well tested; **`main.ts` is entirely untested** (map init, markers, render/frame loop, filters, polling/backoff, view persistence) — it would need a DOM environment plus a maplibre-gl mock; `fetchVehicles` network behavior and `line-colors.ts` are also untested.
