...
- **Coverage**: 94 tests (vehicle 29, motion 34, recorder 18, polyline 4, track 4, hci 5). `logs/` is gitignored; a 15 min recording is ~98 MB. Pure modules (`motion`, `track`, `vehicle`, `hci` parsing) are well tested; **`main.ts` is entirely untested** (map init, markers, render/frame loop, filters, polling/backoff, view persistence) — it would need a DOM environment plus a maplibre-gl mock; `fetchVehicles` network behavior and `line-colors.ts` are also untested.
