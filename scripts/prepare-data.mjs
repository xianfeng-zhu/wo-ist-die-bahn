...
/*
 * CC BY 4.0 attribution, carried INSIDE each generated file.
 *
 * GeoJSON allows extra top-level members, so the notice travels with the data:
 * copy one of these files out of the repo and the credit comes with it. The
 * licence requires naming the creator, linking the licence and the source, and
 * stating that the data was modified. See ATTRIBUTION.md for the full list of
 * modifications this script makes.
 */
const ATTRIBUTION =
  '© VBB Verkehrsverbund Berlin-Brandenburg GmbH, CC BY 4.0 ' +
  '(https://creativecommons.org/licenses/by/4.0/). ' +
  'Source: https://unternehmen.vbb.de/gtfs. ' +
  'MODIFIED: subset selected (rail geometry only; all published line colours), ' +
  'route variants deduplicated and simplified, tracks merged, ' +
  'coordinates rounded. See ATTRIBUTION.md.'
...
try {
  ...
  // Every published colour, not just the rail ones: the app now shows bus, ferry,
  // regional and long-distance too, and VBB publishes a colour for most of them.
  const lfFile = readFileSync(path.join(LF, readdirSync(LF).find(f => f.endsWith('.csv'))), 'latin1')
  const colorEntries = parseLineColors(lfFile)
  console.log(`line colours: ${colorEntries.length} (${colorEntries.filter(([n]) => MODE_ROWS.has(n)).length} are mode fallbacks)`)
  const colorsTs = renderLineColorsTs(colorEntries, ATTRIBUTION)
   ...
   */
  const ownColour = new Map(colorEntries.filter(([n]) => !MODE_ROWS.has(n)))
  ...
  const routesJson = {type: 'FeatureCollection', attribution: ATTRIBUTION, features: routeFeatures}
  const tracksJson = {type: 'FeatureCollection', attribution: ATTRIBUTION, features: trackFeatures}
  const stationsJson = {type: 'FeatureCollection', attribution: ATTRIBUTION, features: stationFeatures}
  ...
  const checks = [
    ...
    ['CC BY attribution present in generated data', [routesJson, tracksJson, stationsJson].every(j => typeof j.attribution === 'string' && j.attribution.includes('CC BY 4.0')) && colorsTs.includes('CC BY 4.0')],
